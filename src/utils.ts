...
const decompressInteger = (stream: Readable) => {
  let result = 0n
...
const compressInteger = (value: number) => {
  const result: bigint[] = []
...
const readString = (stream: Readable) => {
  ...
  return result.toString('utf8') as string
}
...
const readStruct = (stream: Readable) => {
  const result: TDF[] = []
